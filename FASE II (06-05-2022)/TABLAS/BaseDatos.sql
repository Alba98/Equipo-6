...
CREATE TABLE TEMPORADAS(
    COD_TEMPORADA NUMBER(2) GENERATED ALWAYS AS IDENTITY
...
CREATE TABLE JORNADAS (
    ...
    CONSTRAINT JOR_COD_PK PRIMARY KEY(COD_JORNADA),
     CONSTRAINT JOR_TEMP_PK FOREIGN KEY(COD_TEMPORADA) 
        REFERENCES TEMPORADAS(COD_TEMPORADA) ON DELETE CASCADE
);
...
CREATE TABLE PARTIDOS (
    ...
    CONSTRAINT PART_JORD_FK FOREIGN KEY(COD_JORNADA) 
        REFERENCES JORNADAS(COD_JORNADA) ON DELETE CASCADE,
...
CREATE TABLE JUGADORES(
    ...
    CONSTRAINT JUG_PERS_FK FOREIGN KEY(COD_JUGADOR) REFERENCES 
        PERSONAS (COD_PERSONA) ON DELETE CASCADE,
...
CREATE TABLE ASISTENTES (
    ...
    CONSTRAINT ASIS_ENTRE_FK FOREIGN KEY(COD_ENTRE_ASOCIADO) REFERENCES
        ENTRENADORES(COD_ENTRENADOR) ON DELETE CASCADE
);
...
CREATE TABLE ASISTE(
    ...
    CONSTRAINT A_ASIS_FK FOREIGN KEY(COD_ASISTENTE) REFERENCES
        ASISTENTES(COD_ASISTENTE) ON DELETE CASCADE,
    CONSTRAINT A_EQUIPO_FK FOREIGN KEY(COD_EQUIPO) REFERENCES   
        EQUIPOS(COD_EQUIPO) ON DELETE CASCADE
);
...
CREATE TABLE ALMACEN_XML(
    COD_ALMACEN NUMBER(2) GENERATED ALWAYS AS IDENTITY
        (START WITH 1 INCREMENT BY 1),
    result_xml clob,
    CONSTRAINT XML_COD_PK PRIMARY KEY(COD_ALMACEN)
);